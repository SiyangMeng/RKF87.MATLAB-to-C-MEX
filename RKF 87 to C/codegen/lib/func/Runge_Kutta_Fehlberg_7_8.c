/*
 * File: Runge_Kutta_Fehlberg_7_8.c
 *
 ...
 * Return Type  : void
 */
void Runge_Kutta_Fehlberg_7_8(const double b_y0[6], double x, double h, double
  xmax, double tolerance, double y[6], double *out, double *h_next)
{
  int last_interval;
  int i;
  double temp_y0[6];
      ...
      do {
        ...
        if (x < xmax) {
          ...
          while ((!exitg2) && (b_i < 12)) {
            i = b_i + 1;
            for (c_i = 0; c_i < 6; c_i++) {
              temp_y[c_i] = temp_y0[c_i];
            }

            RKF87(temp_y, x, h, err);
            b_err = norm(err);
            if (b_err == 0.0) {
              scale = 4.0;
              exitg2 = true;
              ...
              scale = 0.8 * rt_powd_snf(tolerance * yy / b_err,
                0.14285714285714285);
              if (scale >= 0.125) {
                maxval = scale;
              } else {
...
/*
 * File trailer for Runge_Kutta_Fehlberg_7_8.c
 *
 * [EOF]
 */
